//! File-based suite for the "unfold repeat" refactoring, mirroring the extract
//! and inline suites. Each `tests/repeat/*.repeat` case is annotated source with
//! a `^` caret marking the cursor, then the document expected after unfolding
//! (or `NONE` where the action should not be offered). See `tests/repeat/FORMAT.md`.
...
/// Applies the action's (non-overlapping) edits to `src`, latest offset first.
...
#[test]
fn repeat_cases() {
    let dir = std::path::Path::new("tests/repeat");
    let mut paths: Vec<_> = std::fs::read_dir(dir)
        .expect("tests/repeat/ directory missing")
        .filter_map(|e| {
            let p = e.unwrap().path();
            p.extension().is_some_and(|e| e == "repeat").then_some(p)
        })
        .collect();
    paths.sort();
    assert!(!paths.is_empty(), "no .repeat files found in tests/repeat/");

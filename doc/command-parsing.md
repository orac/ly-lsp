...
A `MusicContext` carries three things: the octave entry (`NoteEntry` — absolute, relative, fixed), the region (`Region` — note music, chord music, non-note), and the language. They differ in *reach*: an entry mode and a region govern a command's body and stop at its closing brace, while a language change outlives it, LilyPond's parser switching note names for the rest of the parse. `Command::music_context` answers with all three and the analyser applies each to its own extent.

Those three are also exactly the state the analyser's walk carries, in the same types, so `handle_command` hands them across by field rather than through a conversion — the walk had its own `Mode`/`Region` pair and a two-way mapping until `MusicContext` grew wide enough to hold the same thing. The one adjustment left is `Region::NoteContext`, which describes the top level of a file and never a command's body, so `Region::in_a_command_body` reads it as `NoteMusic` on the way in. A command that establishes nothing returns `ambient` unchanged, which is why there is no `Inherit` variant to forget to handle; a row says what it establishes as an `Option` per piece, and `\notemode` is the only one with an opinion about the octave entry.

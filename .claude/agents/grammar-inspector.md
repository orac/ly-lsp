...
Use your 'Write' tool to create a temporary file with the construct to test, then run the tool from the ly-lsp crate root (the directory containing `Cargo.toml`):

```bash
cargo run --quiet --example dump_tree -- path/to/file.ly              # from a file
```

You already have permission to run this comand, so avoid more complicated commands, including heredocs. Delete the temporary file when you're done. Run it multiple times on different inputs if you need to.

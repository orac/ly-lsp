...
impl<'a> Analyser<'a> {
    ...
    fn handle_command(&mut self, children: &[Node], start: usize, ambient: Mode) -> usize {
        ...
        match word {
            ...
            "\\clef" => {
                let next = start + 1;
                if matches!(children.get(next).map(|n| n.kind()), Some("symbol" | "string")) {
                    return next + 1;
                }
                next
            }
    ...
    fn read_symbol(&mut self, children: &[Node], start: usize, mode: &mut Mode) -> usize {
        ...
        let Some((note_name, alteration)) = self.language.note(name) else {
            ...
            self.problems
                .push(Problem::NotANote(Span::new(symbol.start_byte(), symbol.end_byte())));
            return start + 1;
        };
    ...
    fn read_chord(&mut self, children: &[Node], start: usize, mode: &mut Mode) -> usize {
        ...
        let end = self.consume_chord_or_tremolo(children, &mut i, after_duration);
        self.push_event(chord.start_byte(), end, EventKind::Chord(notes), duration, written);
        i
    }
    ...
    fn consume_chord_or_tremolo(
        ...
        *i += 1;
        ...
        if !consumed {
            self.problems
                .push(Problem::DanglingColon(Span::new(colon.start_byte(), colon.end_byte())));
            return event_end;
        }
        end
    }
    ...
    fn read_chord_notes(&mut self, chord: Node, mode: Mode) -> Vec<ChordNote> {
        ...
        while k < inner.len() {
            ...
            let Some((note_name, alteration)) = self.language.note(self.text(node)) else {
                self.problems
                    .push(Problem::NotANote(Span::new(node.start_byte(), node.end_byte())));
                k += 1;
                continue;
            };
            k += 1;
...
mod tests {
    ...
    fn chord_absolute_and_post_chord_reference() {
        ...
        let resolved: Vec<(u8, i32)> = notes.iter().map(|n| (n.pitch.note_name, n.pitch.octave)).collect();
    ...
    fn command_arguments_are_not_read_as_notes() {
        ...
        for src in [
            ...
            assert!(analysis.problems.is_empty(), "flagged in {src:?}: {:?}", analysis.problems);
    ...
    fn note_mode_tremolo_is_consumed() {
        ...
        assert_eq!(&"{ c8:32 d }"[analysis.events[0].span.start..analysis.events[0].span.end], "c8:32");
    }
    ...
    fn duration_display_matches_lilypond() {
        assert_eq!(Duration { log: 2, dots: 0, factor: (1, 1) }.to_string(), "4");
        assert_eq!(Duration { log: 3, dots: 1, factor: (3, 2) }.to_string(), "8.*3/2");
        assert_eq!(Duration { log: 0, dots: 0, factor: (4, 1) }.to_string(), "1*4");
        assert_eq!(Duration { log: -1, dots: 0, factor: (1, 1) }.to_string(), "\\breve");
    }
}
